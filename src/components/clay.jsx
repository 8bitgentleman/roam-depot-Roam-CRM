...
const CRMDialog = ({ onClose, isOpen, people }) => {
  ...
  return (
    ...
    //   title="CRM Workspace"
      ...
      style={{ width: '90vw', height: '80vh', maxWidth: 'none', maxHeight: 'none', paddingBottom:"0" }}
    >
      <div className={Classes.DIALOG_BODY} style={{ display: 'flex', height: '100%', margin:"0" }}>
        <div className="left-sidebar" style={{ width: '200px', borderRight: '1px solid #e1e8ed', padding:"20px"}}>
          <h4>CRM Workspace</h4>
          <InputGroup onChange={handleSearchChange} leftIcon="search" placeholder="Search..."  style={{ marginBottom:"10px"}}/>
          <Tabs id="tabs" vertical style={{ height: 'calc(100% - 100px)' }} defaultSelectedTabId="people">
          <Tab id="home" title="Home" panel={<p>Home content</p>} disabled />
            <Tab id="people" title="People" panel={<div />} /> {/* Empty div as panel */}
            ...
            <Tab id="new-person" title="New Person" panel={<p>Add New Person content</p>} disabled />
          </Tabs>
        </div>

   
        {/* Center Section for People List */}
        <div className="people-list-section" style={{ flex: '1', padding: '0 10px 10px 10px', overflowY: 'auto', position: 'relative' }}>
          <div style={{ 
            display: 'flex',
